...
const createSubscription = async (req, res) => {
  if (!process.env.RAZORPAY_KEY_ID) {
    return res.status(503).json({ error: 'Payments not configured yet. Add Razorpay keys to activate.' });
  }
  ...
  try {
    ...
    const { getPlatformRazorpay } = require('../../utils/razorpay');
    ...
    await pool.query(
      `INSERT INTO subscriptions (user_id, razorpay_subscription_id, status)
       VALUES ($1, $2, 'created') ON CONFLICT (razorpay_subscription_id) DO NOTHING`,
      [userId, subscription.id]
    );
    res.json({ subscription_id: subscription.id, status: subscription.status, key_id: process.env.RAZORPAY_KEY_ID });
  } catch (err) {
...
const subscriptionWebhook = async (req, res) => {
  ...
  try {
    if (event.event === 'subscription.activated') {
      ...
      await pool.query(
        `UPDATE subscriptions SET status = 'active', start_date = TO_TIMESTAMP($1), expiry_date = TO_TIMESTAMP($2), updated_at = NOW() WHERE razorpay_subscription_id = $3`,
        [sub.current_start, sub.current_end, subscriptionId]
      );
      const subRecord = await pool.query('SELECT user_id FROM subscriptions WHERE razorpay_subscription_id = $1', [subscriptionId]);
      if (subRecord.rows.length) {
        await pool.query(`UPDATE users SET subscription_status = 'active' WHERE id = $1`, [subRecord.rows[0].user_id]);
      }
    }
    if (event.event === 'subscription.cancelled' || event.event === 'subscription.expired') {
      ...
      const subRecord = await pool.query('SELECT user_id FROM subscriptions WHERE razorpay_subscription_id = $1', [subscriptionId]);
      if (subRecord.rows.length) {
        await pool.query(`UPDATE users SET subscription_status = 'inactive' WHERE id = $1`, [subRecord.rows[0].user_id]);
      }
    }
  } catch (err) {
